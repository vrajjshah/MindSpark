...
{               if ($level != "10" || $round != "1"  ) {
    ...
    {   
         if ($hintvalue=="10") {
            ...
            echo "<script>alert('14th march 2018');</script>";
            # code...
        }
        else if ($hint=="0") {
...
include_once 'includes/psl-config.php';
         $hint=$hint-1;
        $stmt = $mysqli->prepare("UPDATE members set hint='$hint',hintvalue='10' WHERE teamname = ?");
               $stmt->bind_param('s', $_SESSION['teamname'] );
               $stmt->execute();
                $stmt->close(); 
      echo "<script>alert('14th march 2018');</script>";
...
<<main>
    ...
    {
        if(strtolower($_POST['answer']) == 'stephen hawking')
        {
