...
{               if ($level != "7" || $round != "1"  ) {
    ...
    {   
         if ($hintvalue=="7") {
            ...
            echo "<script>alert(' It is a simple question. Please use your mind and try harder.');</script>";
            # code...
        }
        else if ($hint=="0") {
 ...
 -->                    
                ...
                <div class="card-body text-center wow fadeIn" data-wow-delay="0.2s" style="visibility: visible; animation-name: fadeIn; animation-delay: 0.2s;">
                    <!--Title-->
                         ...
                         <p class="card-text" > <h3>If ‘ki Po ra’ stands for ‘Sweets are good’, ‘ma sa Po’ stands for ‘good always triumph’ and ‘la sa ga’ stands for ‘triumph is destined’ then what does ‘ma’ mean ?</h3>  </p>
    ...
    {
        if(strtolower($_POST['answer']) == 'always')
        {
               
                // echo "<script>alert('Correct Answer');</script>";
            
               $insert_stmt = $mysqli->prepare("UPDATE members SET levels='8', date=CURRENT_TIMESTAMP WHERE teamname = ?" );
