...
include_once 'includes/psl-config.php';
         $hint=$hint-1;
        $stmt = $mysqli->prepare("UPDATE members set hint='$hint',hintvalue='15' WHERE teamname = ?");
               $stmt->bind_param('s', $_SESSION['teamname'] );
               $stmt->execute();
                $stmt->close(); 
      echo "<script>alert('helium, neon, argon, krypton, xenon, and radon');</script>";
...
<<main>
 ...
 -->                    
    ...
    {
        if(strtolower($_POST['answer']) == 'nobel peace prize')
        {
               
                // echo "<script>alert('Correct Answer');</script>";
            
               $insert_stmt = $mysqli->prepare("UPDATE members SET levels='16', date=now() WHERE teamname = ?" );
