...
{               if ($level != "11" || $round != "1"  ) {
    ...
    {   
         if ($hintvalue=="11") {
            ...
            echo "<script>alert('Dj Wale Babu, base check karo!!');</script>";
            # code...
        }
        else if ($hint=="0") {
...
include_once 'includes/psl-config.php';
         $hint=$hint-1;
        $stmt = $mysqli->prepare("UPDATE members set hint='$hint',hintvalue='11' WHERE teamname = ?");
               $stmt->bind_param('s', $_SESSION['teamname'] );
               $stmt->execute();
                $stmt->close(); 
      echo "<script>alert('Dj Wale Babu, base check karo!!');</script>";
 ...
 -->                    
                ...
                <div class="card-body text-center wow fadeIn" data-wow-delay="0.2s" style="visibility: visible; animation-name: fadeIn; animation-delay: 0.2s;">
                    <!--Title-->
                         ...
                         <p class="card-text" > <h3>If 0=0, 1=1, 2=2, 3=10 then 81= _____</h3>  </p>
    ...
    {
        if(strtolower($_POST['answer']) == '10000')
        {
